...
- **Web Interface**: http://192.168.178.40:8787
...
- **Workers**: 1 (optimized for 2GB RAM)
...
### Upload a text file:
```bash
curl -F "file=@document.txt" http://192.168.178.40:8787
# Returns: http://192.168.178.40:8787/a3Kf9s.txt
```

### Upload from clipboard/stdin:
```bash
echo "Hello from PotatoStack" | curl -F "file=@-" http://192.168.178.40:8787
# Returns: http://192.168.178.40:8787/b7Jk2p.txt
```

### Upload with custom URL:
```bash
curl -F "file=@report.pdf" -F "url=monthly-report" http://192.168.178.40:8787
# Returns: http://192.168.178.40:8787/monthly-report.pdf
```

### Upload with expiry (1 hour):
```bash
curl -F "file=@secret.txt" -F "expire=1h" http://192.168.178.40:8787
# File will auto-delete after 1 hour
```

### Upload with expiry (1 day):
```bash
curl -F "file=@temp.jpg" -F "expire=1d" http://192.168.178.40:8787
# File will auto-delete after 1 day
```

### Upload with expiry (7 days):
```bash
curl -F "file=@backup.zip" -F "expire=7d" http://192.168.178.40:8787
...
```bash
for file in *.txt; do
    url=$(curl -s -F "file=@$file" http://192.168.178.40:8787)
    echo "$file -> $url"
done
```
...
xclip -selection clipboard -t image/png -o | \
  curl -F "file=@-;filename=screenshot.png" http://192.168.178.40:8787
```

### Download a shared file:
```bash
curl http://192.168.178.40:8787/a3Kf9s.txt
# Or open in browser
```
...
```json
{
  ...
  "RequestURL": "http://192.168.178.40:8787",
  "FileFormName": "file",
  "URL": "$response$"
}
```
...
```bash
curl -F "file=@$1" http://192.168.178.40:8787 | pbcopy
```
...
Max upload size is 50MB. Increase in `rustypaste-config.toml`:
```toml
...
Check cleanup configuration in `rustypaste-config.toml`:
```toml
[paste]
...
Likely a configuration syntax error in `rustypaste-config.toml`
