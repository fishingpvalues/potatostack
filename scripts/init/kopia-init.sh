...
KOPIA_INSECURE_HTTP="${KOPIA_INSECURE_HTTP:-false}"
...
# Generate TLS certificate if needed (skip if using HTTP mode for reverse proxy)
echo "[3/4] Checking TLS certificates..."
if [ "$KOPIA_INSECURE_HTTP" = "true" ]; then
	echo "  ⚠ HTTP mode enabled (KOPIA_INSECURE_HTTP=true)"
	echo "    Use only behind reverse proxy (Tailscale/Traefik)"
elif [ ! -f "/app/config/tls.crt" ] || [ ! -f "/app/config/tls.key" ]; then
...
# Start server with or without TLS based on KOPIA_INSECURE_HTTP
if [ "$KOPIA_INSECURE_HTTP" = "true" ]; then
