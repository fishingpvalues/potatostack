...
echo "======================================"
echo "Stack Validation - SOTA 2025"
echo "Started: $(date)"
echo "======================================"
echo ""

echo "Stack Validation Report" >"$REPORT_FILE"
	...
	if command -v yamllint &>/dev/null; then
		if yamllint -f parsable docker-compose.yml >"$TMP_DIR/yamllint.out" 2>&1; then
			...
			if [ "$errors" -gt 0 ]; then
				...
				cat "$TMP_DIR/yamllint.out" >>"$REPORT_FILE"
...
validate_compose_config() {
	...
	if $DOCKER_COMPOSE_CMD config >$TMP_DIR/compose-config.out 2>&1; then
		echo -e "  ${GREEN}✓${NC} Docker Compose config valid"
		echo "Status: PASSED" >>"$REPORT_FILE"

		# Count services
		local service_count=$(grep -c "^  [a-zA-Z]" $TMP_DIR/compose-config.out 2>/dev/null || echo 0)
		...
		grep -i "error\|warn" $TMP_DIR/compose-config.out >>"$REPORT_FILE" 2>/dev/null || true
...
validate_yaml_parser() {
	...
	if command -v python3 &>/dev/null; then
		if python3 -c "import yaml; yaml.safe_load(open('docker-compose.yml'))" 2>&1 | tee $TMP_DIR/python-yaml.out; then
			echo -e "  ${GREEN}✓${NC} YAML structure valid"
			echo "Status: PASSED" >>"$REPORT_FILE"
		else
			echo -e "  ${RED}✗${NC} YAML structure invalid"
			echo "Status: FAILED" >>"$REPORT_FILE"
			cat $TMP_DIR/python-yaml.out >>"$REPORT_FILE"
			TOTAL_ERRORS=$((TOTAL_ERRORS + 1))
		fi
	else
