...
.PHONY: setup-gluetun-monitor-host
setup-gluetun-monitor-host: ## Setup gluetun-monitor as host systemd service
	@echo "Setting up gluetun-monitor as host service..."
	@echo "This will:"
	@echo "  - Remove containerized monitor (unreliable)"
	@echo "  - Create systemd service for 100% reliable auto-recovery"
	@echo "  - Auto-start on boot"
	@echo "  - Only run when docker is active"
	@echo ""
	@sudo ./scripts/setup/setup-gluetun-monitor-host.sh

.PHONY: gluetun-monitor-status
gluetun-monitor-status: ## Show gluetun-monitor service status
	@systemctl status gluetun-monitor --no-pager

.PHONY: gluetun-monitor-logs
gluetun-monitor-logs: ## Show gluetun-monitor logs
	@journalctl -u gluetun-monitor -f

.PHONY: gluetun-monitor-restart
gluetun-monitor-restart: ## Restart gluetun-monitor service
	@echo "Restarting gluetun-monitor..."
	@sudo systemctl restart gluetun-monitor

.PHONY: gluetun-monitor-stop
gluetun-monitor-stop: ## Stop gluetun-monitor service
	@echo "Stopping gluetun-monitor..."
	@sudo systemctl stop gluetun-monitor

.PHONY: gluetun-monitor-start
gluetun-monitor-start: ## Start gluetun-monitor service
	@echo "Starting gluetun-monitor..."
	@sudo systemctl start gluetun-monitor

.PHONY: gluetun-monitor-enable
gluetun-monitor-enable: ## Enable gluetun-monitor auto-start on boot
	@echo "Enabling gluetun-monitor auto-start..."
	@sudo systemctl enable gluetun-monitor gluetun-monitor-startup.timer

.PHONY: gluetun-monitor-disable
gluetun-monitor-disable: ## Disable gluetun-monitor service
	@echo "Disabling gluetun-monitor..."
