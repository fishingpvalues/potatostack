...
- Android device on same LAN as Le Potato
...
**Server URL**: `http://192.168.178.40:8787`

Replace `192.168.178.40` with your actual HOST_BIND IP.
...
If fails:
```bash
# On Le Potato, check if rustypaste is running
docker ps | grep rustypaste
curl http://192.168.178.40:8787
```
...
**Check file size**: Max 50MB (configurable in `rustypaste-config.toml`)
...
**Wrong IP address**: Verify HOST_BIND in `.env` matches your Le Potato IP

**Not on same network**: DroidyPaste and Le Potato must be on same LAN
...
**Wrong URL format**: DroidyPaste returns full URL like `http://192.168.178.40:8787/abc123.jpg`
...
- Check settings in `rustypaste-config.toml`
